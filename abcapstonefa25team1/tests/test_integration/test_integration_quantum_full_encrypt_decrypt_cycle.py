...
def test_full_encrypt_decrypt_cycle_quantum_shors(tmp_path):
    """
    Integration test covering:
      - RSA key generation
      - File I/O for plaintext/ciphertext
      - Quantum Shor’s factoring and private key derivation
    ...
    # ----- Step 4: Factor N using quantum Shors -----
    shor = Quantum_Shors()
    found_factors = shor.run_shors_algorithm(n, 15)
    assert found_factors is not None, "Quantum Shor's should find factors"
    fp, fq = found_factors
    assert fp * fq == n
...
def test_cli_like_encrypt_decrypt(monkeypatch, tmp_path):
    ...
    # Decrypt path (simulate CLI decrypt command with quantum Shor’s factoring)
    shor = Quantum_Shors()
    factors = shor.run_shors_algorithm(n, 15)
...
def test_handles_unfactorable_prime_rsa_key(tmp_path):
    ...
    e = 3
    shor = Quantum_Shors()
    result = shor.run_shors_algorithm(prime_modulus, 15)
